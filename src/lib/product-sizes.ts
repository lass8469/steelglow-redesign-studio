export const productSizes: Record<string, string[]> = {
  "drybag-i": ["1000g", "2000g"],
  "drybag-iii": ["1g", "5g", "10g", "25g", "50g", "100g", "140g", "250g", "500g", "1000g", "2000g", "Custom"],
  "silica-gel": ["1g", "5g", "10g", "12.5g", "25g"],
  "molecular-sieve": ["2g 3Å", "2g 4Å"],
  "calcium-chloride": ["4x100g", "6x100g", "6x125g", "12x100g", "2000g"],
  "dunnage-bags": ["60x120cm", "90x120cm", "90x150cm", "90x180cm", "90x220cm", "120x180cm", "120x240cm"],
  "edge-protectors": ["Custom"],
  "anti-slip": ["Euro-pallet (740x1140mm)", "1000x1000mm", "Custom"],
  "stabustrap": ["1200mm Standard", "1200mm Heavy", "1400mm Standard", "1400mm Heavy"],
  "retail": ["1000g"],
  "datalogger": ["Temperature & Humidity"],
};
