...
const CalciumChlorideProduct = () => {
  ...
  const specifications = [
    ...
    { label: t("calcium.spec.duration"), value: t("calcium.spec.durationValue") },
    { label: t("calcium.spec.temp"), value: t("calcium.spec.tempValue") },
    { label: t("calcium.spec.shelf"), value: t("calcium.spec.shelfValue") },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <MoistureBackground variant="large" className="bg-background"><section className="py-12 lg:py-20"><div className="container mx-auto px-4 lg:px-8"><div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div className="relative group"><div className="absolute -inset-4 bg-gradient-to-r from-primary/20 to-transparent rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"/><div className="relative aspect-square rounded-xl overflow-hidden bg-card border border-border"><img src={productCalciumChloride} alt="Calcium Chloride Absorber Blanket" loading="lazy" className="w-full h-full object-contain p-8 transition-transform duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-card/80 via-transparent to-transparent"/><div className="absolute bottom-6 left-6"><span className="inline-block px-3 py-1 text-xs font-semibold tracking-wider uppercase bg-primary/90 text-primary-foreground rounded-full">{t("calcium.badge")}</span></div></div></div>
        <div className="space-y-6"><div><span className="inline-block px-4 py-1.5 mb-4 text-xs font-semibold tracking-widest uppercase bg-primary/10 text-primary border border-primary/20 rounded-full">{t("calcium.tagline")}</span><h1 className="text-4xl lg:text-5xl font-black tracking-tight mb-4">{t("calcium.title")}<span className="text-gradient">{t("calcium.titleHighlight")}</span></h1><p className="text-lg text-muted-foreground leading-relaxed">{t("calcium.description")}</p></div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6"><Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold px-8 glow-orange" asChild><a href="mailto:contact@dry-bag.com">{t("productPage.requestQuote")}</a></Button><SpecSheetDialog productKey="calcium-chloride" productName="Calcium Chloride" /></div>
        </div>
