...
const StabustrapProduct = () => {
  ...
  const specifications = [
    ...
    { label: t("stabustrap.spec.application"), value: t("stabustrap.spec.applicationValue") },
    { label: t("stabustrap.spec.reusability"), value: t("stabustrap.spec.reusabilityValue") },
    { label: t("stabustrap.spec.recyclability"), value: t("stabustrap.spec.recyclabilityValue") },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <MoistureBackground variant="large" className="bg-background"><section className="py-12 lg:py-20"><div className="container mx-auto px-4 lg:px-8"><div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div className="relative group"><div className="absolute -inset-4 bg-gradient-to-r from-primary/20 to-transparent rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"/><div className="relative aspect-square rounded-xl overflow-hidden bg-card border border-border"><img src={productStabustrap} alt="Stabustrap Sustainable Strapping System" className="w-full h-full object-contain p-8 transition-transform duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-card/80 via-transparent to-transparent"/><div className="absolute bottom-6 left-6"><span className="inline-block px-3 py-1 text-xs font-semibold tracking-wider uppercase bg-primary/90 text-primary-foreground rounded-full">{t("stabustrap.badge")}</span></div></div></div>
        <div className="space-y-6"><div><span className="inline-block px-4 py-1.5 mb-4 text-xs font-semibold tracking-widest uppercase bg-primary/10 text-primary border border-primary/20 rounded-full">{t("stabustrap.tagline")}</span><h1 className="text-4xl lg:text-5xl font-black tracking-tight mb-4">{t("stabustrap.title")}<span className="text-gradient">{t("stabustrap.titleHighlight")}</span></h1><p className="text-lg text-muted-foreground leading-relaxed">{t("stabustrap.description")}</p></div>
