  ...
  const textReviews = [
    {
      ...
      highlight: t("socialProof.review1.highlight"),
      rating: 5,
              ...
              {[...Array(4)].map((_, setIndex) => (
                <div key={setIndex} className="flex items-center gap-8 pr-8">
                  {trustedLogos.map((logo, index) => (
                    <div
                      key={`${setIndex}-${index}`}
                      className="flex items-center justify-center gap-2 h-20 w-40 flex-shrink-0 px-4 bg-card/50 border border-border/50 rounded-sm hover:border-primary/30 transition-all group hover:bg-card/80"
                    >
                      {logo.logo ? (
                        ...
                        <span className="font-bold text-sm tracking-tight text-muted-foreground group-hover:text-foreground transition-colors">
                          {logo.name}
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-4">
                  {review.image ? (
                    <div className="w-28 h-28 rounded-full overflow-hidden flex-shrink-0">
                      <img src={review.image} alt={review.name} className={`w-full h-full object-cover ${review.name === "Christina Rosenkvist" ? "scale-[1.4] object-[center_30%]" : "object-top scale-[1.25]"}`} />
                    </div>
                  ) : (
