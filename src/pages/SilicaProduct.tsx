...
const SilicaProduct = () => {
  ...
  const specifications = [
    ...
    { label: t("silica.spec.indicator"), value: t("silica.spec.indicatorValue") },
    { label: t("silica.spec.temp"), value: t("silica.spec.tempValue") },
    { label: t("silica.spec.shelf"), value: t("silica.spec.shelfValue") },
    { label: t("silica.spec.cert"), value: t("silica.spec.certValue") },
  ];
  const sizes = [
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <MoistureBackground variant="large" className="bg-background"><section className="py-12 lg:py-20"><div className="container mx-auto px-4 lg:px-8"><div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div className="relative group"><div className="absolute -inset-4 bg-gradient-to-r from-primary/20 to-transparent rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"/><div className="relative aspect-square rounded-xl overflow-hidden border border-border bg-gradient-to-br from-muted/80 via-muted/40 to-accent/30"><div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_70%,hsl(var(--primary)/0.08),transparent_50%)]"/><img src={productSilica} alt={t("silica.imgAlt")} loading="lazy" className="w-full h-full object-contain p-8 transition-transform duration-700 group-hover:scale-105 relative z-10"/><div className="absolute inset-0 bg-gradient-to-t from-card/60 via-transparent to-transparent z-20"/><div className="absolute bottom-6 left-6 z-30"><span className="inline-block px-3 py-1 text-xs font-semibold tracking-wider uppercase bg-primary/90 text-primary-foreground rounded-full">{t("silica.badge")}</span></div></div></div>
        <div className="space-y-6"><div><span className="inline-block px-4 py-1.5 mb-4 text-xs font-semibold tracking-widest uppercase bg-primary/10 text-primary border border-primary/20 rounded-full">{t("silica.tagline")}</span><h1 className="text-4xl lg:text-5xl font-black tracking-tight mb-4">{t("silica.title")}<span className="text-gradient">{t("silica.titleHighlight")}</span></h1><p className="text-lg text-muted-foreground leading-relaxed">{t("silica.description")}</p></div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6"><Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold px-8 glow-orange">{t("productPage.requestQuote")}</Button><SpecSheetDialog productKey="silica-gel" productName="Silica Gel" /></div>
        </div>
